...
import { FiTrendingUp, FiArrowUpRight, FiArrowDownRight, FiShoppingBag } from 'react-icons/fi';
import { useOneProductSalesEvolution } from '@/hooks/useOneProductSalesEvolution';
...
export function ProductSalesTab({ code13ref }: ProductSalesTabProps) {
  const [interval, setInterval] = useState<IntervalType>('day');
  const [showMargin, setShowMargin] = useState(true);
  
  // Utiliser notre hook personnalisé
  const { data: salesData, isLoading, error } = useOneProductSalesEvolution(code13ref, interval);
