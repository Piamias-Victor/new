// src/components/dashboard/products/ProductSalesInsights.tsx
...
export function ProductSalesInsights({ data, interval }: ProductSalesInsightsProps) {
  ...
  const insights = useMemo(() => {
    ...
    try {
      ...
      const sortedByRevenue = [...dataPoints].sort((a, b) => Number(b.total_revenue) - Number(a.total_revenue));
      ...
      for (const point of dataPoints) {
        totalRevenue = totalRevenue + Number(point.total_revenue);
        totalMargin = totalMargin + Number(point.total_margin);
      }
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-4">
      ...
      <div className="bg-white dark:bg-gray-800/70 p-4 rounded-lg border border-gray-100 dark:border-gray-700">
        ...
        <div className="text-xl font-bold text-gray-900 dark:text-white mb-1">
          {formatCurrency(Number(insights.bestPeriod.total_revenue))}
        </div>
      ...
      <div className="bg-white dark:bg-gray-800/70 p-4 rounded-lg border border-gray-100 dark:border-gray-700">
        ...
        <div className="text-xl font-bold text-gray-900 dark:text-white mb-1">
          {formatCurrency(Number(insights.worstPeriod.total_revenue))}
        </div>
        <div className="text-xs text-gray-500 dark:text-gray-400">
          <FiCalendar size={12} className="inline mr-1" />
