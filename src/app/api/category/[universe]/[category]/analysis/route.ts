...
export async function POST(
  ...
  try {
    ...
    try {
      ...
      const baseParams = [startDate, endDate, universe, category];
      ...
      // Requête pour obtenir le chiffre d'affaires total de la catégorie
      const revenueQuery = `
...
`;
      ...
      // Obtenir les top produits du laboratoire sélectionné pour cette catégorie
      ...
      console.log(`Produits du laboratoire trouvés pour la catégorie: ${selectedLabProducts.length}`);
      
      // Obtenir les top produits des autres laboratoires pour cette catégorie
      ...
      console.log(`Produits des autres laboratoires trouvés pour la catégorie: ${otherLabProducts.length}`);
...
export async function GET(
  ...
  try {
    ...
    console.error('Erreur lors de l\'analyse de la catégorie (GET):', error);
    return NextResponse.json(
      { error: 'Erreur serveur', details: error instanceof Error ? error.message : 'Unknown error' },
      { status: 500 }
    );
