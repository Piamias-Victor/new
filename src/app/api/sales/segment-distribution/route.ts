...
// Conserve la méthode GET pour la compatibilité
export async function GET(request: Request) {
  try {
    const { searchParams } = new URL(request.url);
    const startDate = searchParams.get('startDate');
    const endDate = searchParams.get('endDate');
    const segmentType = searchParams.get('segmentType') || 'universe';
    ...
    return await processSegmentDistribution(startDate, endDate, segmentType, pharmacyIds, code13refs);
  } catch (error) {
...
// Ajoute une méthode POST pour gérer les grandes listes de codes EAN
export async function POST(request: Request) {
  try {
    const body = await request.json();
    const { startDate, endDate, segmentType = 'universe', pharmacyIds = [], code13refs = [] } = body;
...
// Fonction commune pour traiter les données de distribution par segment
async function processSegmentDistribution(
  startDate: string, 
  endDate: string, 
  segmentType: string,
  pharmacyIds: string[],
    ...
    query = `
      WITH product_data AS (
        SELECT 
          ...
          p."TVA",
          COALESCE(g.${segmentType}, 'Autre') AS segment_value
        FROM 
          data_sales s
        JOIN 
